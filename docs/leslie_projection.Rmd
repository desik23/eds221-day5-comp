...
# Creates a vector of years from 0 to 8
time_yr <- seq(from = 0, to = 8, by = 1)

# creates a matrix with years as rows and number of population parameters as columns
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# First row is initial vector
proj_year[1, ] <- fish_year0

# Computes next years output based on previous year
